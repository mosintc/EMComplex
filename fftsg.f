...
!
      ...
      if (nw .gt. 2) then
          ...
          if (nwh .eq. 4) then
              ...
              call makeipt(nw, ip)
              w(2) = 0.5d0 / dcos(delta * 2.0d0)
              w(3) = 0.5d0 / dcos(delta * 6.0d0)
              do j = 4, nwh - 4, 4
                  w(j) = dcos(delta * j)
                  w(j + 1) = dsin(delta * j)
                  w(j + 2) = dcos(3.0d0 * delta * j)
                  w(j + 3) = -dsin(3.0d0 * delta * j)
              end do
          end if
          nw0 = 0
          do while (nwh .gt. 2)
...
!
      ...
      if (nc .gt. 1) then
          nch = nc / 2
          delta = datan(1.0d0) / nch
          c(0) = dcos(delta * nch)
          c(nch) = 0.5d0 * c(0)
          do j = 1, nch - 1
              c(j) = 0.5d0 * dcos(delta * j)
              c(nc - j) = 0.5d0 * dsin(delta * j)
          end do
      end if
      end
!
